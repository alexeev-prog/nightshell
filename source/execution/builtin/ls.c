...
void check_permissions(struct stat fileStat) {
     ...
     * Output: void*/
    printf(YELLOW);
    printf((S_ISDIR(fileStat.st_mode)) ? "d" : ((S_ISLNK(fileStat.st_mode)) ?: "-"));
    printf(GREEN);
 ...
 */
void list_files(
    const char* dir_path, bool show_permissions, bool show_time, bool show_hidden, bool list_show) {
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        } else if ((file_stat.st_mode & S_IXUSR) || (file_stat.st_mode & S_IXGRP)
                   || (file_stat.st_mode & S_IXOTH))
        {
            if (!list_show) {
                ...
                print_styled(GREEN, entry->d_name, "  ");
                   ...
                   && (strstr(entry->d_name, ".png") != NULL || strstr(entry->d_name, ".jpg") != NULL
                       || strstr(entry->d_name, ".jpeg") != NULL || strstr(entry->d_name, ".svg") != NULL
...
int sgls(char** args) {
    ...
    list_files(dir_path, show_permissions, show_time, show_hidden, show_list);
